...
public class XQueryRunConfigurationProducer extends RuntimeConfigurationProducer implements Cloneable {
    ...
    private RunnerAndConfigurationSettings prepareSettings(ConfigurationContext context, Project project,
        ...
        setRunnerSettings(vFile, configuration);
        setupConfigurationModule(context, configuration);
        settings.setEditBeforeRun(true);
        return settings;
    }

    private void setRunnerSettings(VirtualFile vFile, XQueryRunConfiguration configuration) {
    ...
    private RunnerAndConfigurationSettings getExistingConfigurationForLocation(Location location, PsiFile psiFile,
        ...
        for (RunnerAndConfigurationSettings existingConfiguration : existingConfigurations) {
            final XQueryRunConfiguration appConfiguration = (XQueryRunConfiguration) existingConfiguration
                    .getConfiguration();
            if (isForTheSameFile(psiFile, appConfiguration) && isForTheSameModule(location,
                    appConfiguration) && hasTheSameVariablesNumber(psiFile, appConfiguration)) {
    ...
    private boolean hasTheSameVariablesNumber(PsiFile file, XQueryRunConfiguration appConfiguration) {
        XQueryFile xQueryFile = (XQueryFile) file;
        return getExternalVariables(xQueryFile).equals(appConfiguration.getVariables());
    }
