...
import org.intellij.xquery.runner.rt.XQJTypes;
 ...
 * Time: 14:17
 */
public class XQueryRunVariableDialog extends DialogWrapper {
    ...
    private boolean isValidConfiguration() {
        boolean nameFieldPopulated = name.getComponent().getText() != null && name.getComponent().getText().length() > 0;
        boolean typeFieldPopulated = type.getComponent().getSelectedItem() != null;
        return nameFieldPopulated && typeFieldPopulated;
    }

    public XQueryRunVariableDialog() {
        super(true);
    ...
    private void populateTypesList() {
        for (String type : XQJTypes.getAll()) {
            typesModel.add(type);
        }
    }
